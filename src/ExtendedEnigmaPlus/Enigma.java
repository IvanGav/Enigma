...
public class Enigma {
    /*
    Third generation of Enigma. Can be easily configured and more flexibility with rotors, characters encoded, etc.
    */
    ...
    public Enigma(String charList, String swaps, Reflector reflector, Rotor[] rotors, int[] rotorsRot) {
        chars = charList;
        pb = new PlugBoard();
        pb.addSwap(swaps);
        rb = new RotorBoard(chars.length());
        rb.setReflector(reflector);
        for(Rotor r: rotors) {
            rb.addRotor(r);
        }
        setRotorsRotation(rotorsRot);
